	...
	<body>
				...
				<div class="col-xs-12 padhor-0">
					...
					<div class="force-vertical-align black-bg-filter width-100per">
						<div class="force-vertical-align">
							<h1 class="text-center font-1-8em text-uppercase font-700 color-fff mtop-0 mbottom-5 text-shadow font-1-3em-xs mbottom-30-xs">Cachorros e gatos perdidos</h1>
							<p class="mbottom-30 text-center font-300 font-1-5em color-fff text-shadow hidden-xs">Veja a lista de todos os animais perdidos</p>
						</div>
					</div>
				</div>
			</div>
			...
			<div class="">
				<div class="row">
					<div class="col-xs-12 col-md-3 col-sm-3 col-lg-2 mbottom-30">
						<form class="" action="animais-perdidos.php" method="GET">
		...
		<script>
			$(function(){
				...
				<?php
				$urlGetDoacoes = 'http://31.220.53.123:8080/luckypets-servidor/api/anuncio/get-perdidos';
				if (isset($_GET['tipo']) || isset($_GET['porte']) || isset($_GET['sexo']) || isset($_GET['idade']) || isset($_GET['castrado'])) {
					...
					$urlGetDoacoes = 'http://31.220.53.123:8080/luckypets-servidor/api/anuncio/get-perdido-filtered' . $params;
				} ?>
				$.ajax({
				    type: 'GET',
				    crossOrigin: true,
				    ...
				    success:function(x){
					    ...
					    for (i = 0; i < x.length; i++) {
							...
							html += '       </div>';
							//html += '		<img src="http://31.220.53.123:8080/luckypets-servidor/api/file/perdido/' + x[i].id + '/' + x[i].animal.imagens[0] + '" alt="' + x[i].animal.nome + '" class="img-responsive center-block" style="max-height: 206px;">';
